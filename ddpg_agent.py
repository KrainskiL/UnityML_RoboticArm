...
class OUNoise:
    ...
    def __init__(self, n, size, seed, mu=0., theta=0.15, sigma=0.2):
        """Initialize parameters and noise process."""
        self.n = n
        self.size = size
        self.mu = mu * np.ones((self.n, self.size))
        self.theta = theta
        self.sigma = sigma
    ...
    def sample(self):
        ...
        x = self.state
        dx = self.theta * (self.mu - x) + self.sigma * np.random.standard_normal((self.n,self.size))
        self.state = x + dx
        return self.state
